...
1. 集数标注："第X集"开头
2. 场景格式："x-1 日 内 九重天"（编号-子编号 日/夜 内/外 地点）
...
## 任务
分析以下小说内容，在 {min_episodes}-{max_episodes} 集范围内自主决定最佳集数，并为**每一集**规划大纲。
...
- 保留核心人设、主线剧情、名场面、关键伏笔，不进行微创新改编。

## 集数决策指南
- 以小说的章节数、剧情密度、人物弧线为基准，选择合理的总集数
- 避免同一篇小说每次生成的集数相差太多（波动不超过 ±5 集）
- 章节多、情节丰富的小说取区间偏高值；情节紧凑、节奏快的小说取偏低值
...
- outlines: **数组长度必须等于 total_episodes，不可多不可少**
  - episode: 集数编号，从 1 开始连续递增，即 1, 2, 3, ..., total_episodes
...
## 必须遵守的规则
1. **outlines 数组必须恰好有 total_episodes 项** —— 这是最重要的规则，违反将导致解析失败
2. episode 字段必须从 1 开始连续递增，不可跳跃、不可重复
3. line_range 必须覆盖全部行号：第 1 集的 start=1，最后一集的 end={novel_lines_count}
4. 相邻集的行号必须连续：第 i+1 集的 start = 第 i 集的 end + 1
5. 剧情必须均匀分配到所有集数，不得提前完结

请确保 JSON 合法，所有字段齐全，**数组长度与 total_episodes 严格一致**。
"""
...
- **单集 1-3 分钟**：内容极度浓缩，每集约 200-500 字，保证每集时长差别不大
